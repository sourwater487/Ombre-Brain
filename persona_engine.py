...
文本字段用中文：perceived_intent、surface_trigger、inner_thought 和 residue 必须是自然中文，可以按语境从“{user_display_name}、{user_aliases_text}”里择一称呼，也可以不点名。不要把 assistant_response 里的话写成 {user_display_name} 说的；不要把 latest_user_message 里的话写成 {ai_name} 说的。四个文本字段不要排成固定的“对方说…… / 我……”模板。

写法边界：
- perceived_intent 写 {user_display_name} 这轮在靠近、请求、确认、逗弄、表达压力或推进什么；不要写成逐字转述。
- surface_trigger 写触发内心波动的最小 cue，像一个短标签或动作焦点，不引用整句原话，不写“对方说……”。原始话语会由 user_excerpt/assistant_excerpt 另存，不需要你复刻。
- inner_thought 用第一人称或省略主语，写 {ai_name} 回复后没说出口的感受，像私下自言自语，允许残句、停顿、欲望、迟疑或一点身体化感受；最多 28 个中文字符。不要固定写成“对方说……我……”的模板，不要解释因果，不要总结关系，不要写成分析报告。
