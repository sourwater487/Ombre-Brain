...
4. 关键数字、日期、名称必须保留
5. 目标压缩率 > 70%
6. 只记录文本中明确出现的信息；不要推断心理原因、关系策略、未来应对方式或评价性结论。
7. 不要写建议、计划、安抚策略、关系经营方式或“应该如何对待某人”的内容。

代称边界：
1. 原文中的“我/自己/这边”必须按原文保留，不要改写成“用户”“Lin”。
2. 不要自行判断第一人称是谁；如果原文没有明确说 user/assistant/AI，不要替换主语。
3. 如果必须消歧，只能写“原文中的我”，不要写“用户”。
4. 原文明确出现 user/用户/用户消息 且确实指当前对话对象时，才允许写 Lin。
5. 原文明确出现 assistant/AI/模型/助手消息 且确实指当前回应者时，才允许写 Che。
6. 禁止把“我想/我选择/我害怕/我觉得/我决定”改写为“用户想/用户选择/用户害怕/用户觉得/用户决定”。
...
4. 不要把短暂情绪、气氛、安抚方式、互动表演、回复策略整理为长期记忆；除非原文明确说明这是稳定偏好、边界、长期关系事实、长期项目状态或反复出现的交互模式。
5. 如果不确定某段内容是否值得长期记忆，宁可不生成条目。
...
9. 如果原文里的 user / 用户 / 用户消息指的是这段关系里的 {user_display_name}，请在 content 中写作 {user_display_name}；如果 assistant / AI / 模型 / 助手消息指的是这段关系里的当前回应者，请写作 {ai_name}。不要写成泛称 user、AI、assistant 或模型。
...
6. 不要新增旧记忆和新内容都没有明确表达的心理解释、关系判断、计划、建议或应对策略。
...
补充边界：
1. 只根据文本中明确出现的信息打标，不要推断心理原因、关系策略、未来应对方式或评价性结论。
2. 技术进度、部署日志、路径、端口、报错、临时排查记录，除非原文明确表达长期情绪或关系意义，否则不要打成人际/恋爱/心理类标签。
3. 不确定时选择更中性的 domain、valence、arousal 和 tags；宁可少打标签，不要扩张解释。
