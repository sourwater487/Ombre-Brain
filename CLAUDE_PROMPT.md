...
| `grow` | 当用户明确要求整理一大段内容，或你已经从日记里摘出一段包含多个长期记忆点的片段时调用。不要把整篇日终日记默认丢给 `grow`；Che-diary 保存完整故事，Ombre 只保存少量长期记忆卡 |
...
- **日记摘记**：日终原文写入 Che-diary；只有出现长期有用的片段时，才用 `hold` 或 `grow` 写入 Ombre
...
- 整篇日记、一天流水、完整情绪过程 → 写入 Che-diary，不要默认 `grow`
